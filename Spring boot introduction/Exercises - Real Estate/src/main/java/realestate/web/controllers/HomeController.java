...
import realestate.conficurations.Constants;
...
public class HomeController {
    ...
    private String prepareView() throws IOException {
        ...
        List<OfferViewModel> offers = this.offerService
                .findAllOffers()
                .stream()
                .map(o -> this.modelMapper
                        .map(o, OfferViewModel.class))
                .collect(Collectors.toList());
        if (offers.size() == 0) {
            offertHtmlContent
                    .append("<div class=\"apartment\" style=\"border:red solid 1px\">")
                    .append("There are not any offers!")
                    .append(" </div>");
        ...
        return this.reader
                .readHtmlFile(Constants.INDEX_HTML_PATH_FILE)
                .replace("{{offers}}", offertHtmlContent.toString().trim());
    }
